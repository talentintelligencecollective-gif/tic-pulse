...
export default function ArticleCard({ article, index, user, onLike, onBookmark, onShare, isLiked, isBookmarked, isSelected, onToggleSelect }) {
  ...
  // Load comments when expanded (comment count comes from articles_feed view — no N+1)
  useEffect(() => {
    ...
    async function loadComments() {
      try {
        const { data } = await supabase
          .from("comments")
          .select("id, user_id, article_id, text, user_name, created_at")
  ...
  // Submit comment — no longer stores user_email (privacy fix)
  const handleSubmitComment = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("comments").insert({
        ...
        user_name: userName,
      }).select("id, user_id, article_id, text, user_name, created_at").single();
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", paddingTop: "50%", overflow: "hidden" }}>
        ...
        />
        {/* Hidden probe for broken image detection — lazy loaded */}
        {article.image_url && !imgError && <img src={article.image_url} alt="" loading="lazy" style={{ display: "none" }} onError={() => setImgError(true)} />}
        {!hasImage && (
