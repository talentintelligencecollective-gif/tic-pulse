...
const REGIONS = ["All", "North America", "Europe", "Asia Pacific", "Middle East", "Africa"];
...
// ═══════════════════════════════════════════════
//  PULSE APP — v3: region filters + personalisation
// ═══════════════════════════════════════════════
  ...
  return (
    <div style={{ minHeight: "100dvh", maxWidth: "480px", margin: "0 auto", position: "relative" }}>
      ...
      <Header
        ...
        activeCategory={activeCategory} activeRegion={activeRegion}
        sortMode={sortMode}
      ...
      <main style={{ position: "relative", zIndex: 1 }}>
        {activeTab === "feed" && (
          <FeedView
            ...
            sortMode={sortMode} activeRegion={activeRegion}
            onLike={handleLike} onBookmark={handleBookmark} onShare={handleShare}
            onToggleSelect={handleToggleSelect}
            onClearFilters={() => { setActiveCategory("All"); setActiveRegion("All"); setSearchQuery(""); setSearchOpen(false); }}
...
// ═══════════════════════════════════════════════
//  HEADER — with region pills + sort toggle
// ═══════════════════════════════════════════════

function Header({ searchOpen, searchQuery, activeCategory, activeRegion, sortMode, searchInputRef, user, activeTab, onLogout, onToggleSearch, onSearchChange, onCategoryChange, onRegionChange, onSortModeChange }) {
    ...
    <header style={{ position: "sticky", top: 0, zIndex: 100, background: "#000", borderBottom: "1px solid #222" }}>
      ...
      {showCategories && (
        <>
          ...
          {/* Region pills */}
          <div style={{ display: "flex", gap: "6px", padding: "8px 16px 10px", overflowX: "auto", scrollbarWidth: "none" }}>
            <span style={{ fontSize: "9px", fontWeight: 700, color: "#555", letterSpacing: "1px", alignSelf: "center", flexShrink: 0, marginRight: "2px" }}>REGION</span>
            {REGIONS.map((reg) => {
              const isActive = activeRegion === reg;
              return (
                <button key={reg} onClick={() => onRegionChange(reg)} style={{
                  padding: "4px 12px", borderRadius: "16px", whiteSpace: "nowrap",
                  fontSize: "10px", fontWeight: 600, letterSpacing: "0.2px",
                  border: isActive ? "1px solid rgba(0,180,216,0.35)" : "1px solid #222",
                  background: isActive ? "rgba(0,180,216,0.08)" : "transparent",
                  color: isActive ? "#00b4d8" : "#777",
                  transition: "all 0.2s",
                }}>{reg}</button>
              );
            })}
...
// ═══════════════════════════════════════════════
//  FEED VIEW — with sort mode indicator
// ═══════════════════════════════════════════════

function FeedView({ articles, loading, error, searchQuery, likedIds, bookmarkedIds, selectedIds, trendingTags, user, sortMode, activeRegion, onLike, onBookmark, onShare, onToggleSelect, onClearFilters, onSearchTag, onRetry }) {
  const hasSelections = selectedIds.size > 0;
  const hasActiveFilters = searchQuery || activeRegion !== "All";
  ...
  return (
    <div style={{ padding: hasSelections ? "12px 12px 180px" : "12px 12px 110px" }}>
      ...
      {hasActiveFilters && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <span style={{ fontSize: "12px", color: "#888" }}>Filtering:</span>
            {searchQuery && <span style={{ fontSize: "12px", fontWeight: 700, color: "#00e5a0" }}>{searchQuery}</span>}
            {activeRegion !== "All" && <span style={{ fontSize: "11px", fontWeight: 600, color: "#00b4d8", padding: "2px 8px", background: "rgba(0,180,216,0.1)", borderRadius: "8px" }}>{activeRegion}</span>}
          </div>
      ...
      {/* Sort mode label */}
      {sortMode === "foryou" && articles.length > 0 && (
        <div style={{ padding: "0 4px 8px", fontSize: "11px", color: "#666" }}>
          Personalised feed · {articles.length} article{articles.length !== 1 ? "s" : ""}
        </div>
      )}
